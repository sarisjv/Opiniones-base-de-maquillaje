...
def main():
    ...
    with tab_avanzado:
        ...
        with subtab2:
            ...
            df['Sentimiento'] = df['Opinió'].apply(lambda x: analizar_sentimiento(x)[0])
            df['Puntaje'] = df['Opinió'].apply(lambda x: analizar_sentimiento(x)[1])
            ...
            if opcion == "Ver todas las opiniones":
                ...
                for categoria in ["Positivo", "Neutral", "Negativo"]:
                    ejemplos = df[df['Sentimiento'] == categoria]['Opinió'].head(2)
